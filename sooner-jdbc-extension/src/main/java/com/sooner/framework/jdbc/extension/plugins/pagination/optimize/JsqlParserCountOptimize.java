 ...
 * the License.
 */
package com.sooner.framework.jdbc.extension.plugins.pagination.optimize;
        ...
        try {
            ...
            for (SelectItem item : plainSelect.getSelectItems()) {
                if (item.toString().contains(StringPool.QUESTION_MARK)) {
                    sqlInfo.setSql(SqlParserUtils.getOriginalCountSql(selectStatement.toString()));
                    return sqlInfo;
                }
            }
            ...
            // 无法优化使用原 SQL
            sqlInfo.setSql(SqlParserUtils.getOriginalCountSql(sql));
            return sqlInfo;
        }
    }
